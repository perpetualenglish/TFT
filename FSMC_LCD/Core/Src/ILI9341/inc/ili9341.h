...
//A12×÷ÎªÊý¾ÝÃüÁîÇø·ÖÏß  ÉèÖÃÊ±STM32ÄÚ²¿»áÓÒÒÆÒ»Î»¶ÔÆë	    
#define  CMD_BASE     ((uint32_t)(0x6C000000 | 0x00001FFE))
#define  DATA_BASE    ((uint32_t)(0x6C000000 | 0x00002000))

#define LCD_CmdWrite(command)	*(volatile uint16_t *) (CMD_BASE) = (command)
#define LCD_DataWrite(data)		*(volatile uint16_t *) (DATA_BASE) = (data)
#define	LCD_StatusRead()		*(volatile uint16_t *) (CMD_BASE) //if use read  Mcu interface DB0~DB15 needs increase pull high
#define	LCD_DataRead()			*(volatile uint16_t *) (DATA_BASE) //if use read  Mcu interface DB0~DB15 needs increase pull high
